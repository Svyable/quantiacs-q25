...
- Canonical latest: `frontier_20260916p`.
- Measured awaiting canonical ingest: **0**.
- Frozen and still unmeasured: **0**.
- Failed frozen forward gates: **1**.
- Current bottleneck: **EVIDENCE_DEPTH** — Canonical measurement is ahead of the latest full matrix packet.
...
- **selection_transfer · RESEARCH_OPTIMISM_OBSERVED** — Median Dev−Research SR@12 is -1.565; 0/9 comparable cells held or improved. _This describes the recent comparable cells, not a universal shrinkage coefficient._
...
- **evidence_freshness · EVIDENCE_DEPTH_BOTTLENECK** — Canonical measurement is ahead of the latest full matrix packet. _Pipeline state is derived from committed repository artifacts only._

## Research-budget route

**Next:** `DEEPEN_MATRIX_EVIDENCE` `frontier_20260916p` — Latest committed campaign is not represented by a full canonical matrix packet.

Current-state backlog: none. Recompute after the next evidence transition.
