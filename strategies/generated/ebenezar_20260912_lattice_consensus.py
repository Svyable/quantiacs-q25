...
def strategy(data,params=None,mode="base"):
    ...
    c,l,r=_base(data); v=_std(r,21,10); s=np.sqrt(365.0)*_sma(r,7,5)/(v+EPS); sma18=_sma(c,18,10); sma50=_sma(c,50,25); trend=sma18/(sma50+EPS)-1.0; mom=c/c.shift(time=14)-1.0; peak=c.rolling(time=42,min_periods=20).max(); dd=c/(peak+EPS)-1.0; ms=_mean(s,l); mv=_mean(v,l)
