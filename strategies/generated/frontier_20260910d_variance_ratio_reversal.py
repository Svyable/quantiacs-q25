"""frontier_20260910d_variance_ratio_reversal — PENDING RESEARCH STRATEGY.
Experiment: frontier_20260910d_variance_ratio_reversal
Preregistration SHA256: 37e486539e7388879e37392ef6f5dcf44eff142811391ee9595466a3618becca
Mechanism: Negative serial dependence predicts reversal of a recent relative loss during a positive market trend.
Nearest incumbent: mean_reversion / V10 residual momentum / Frontier-C signed risk transition
...
def signals(data, window, mode="base"):
    ...
    # Both variance estimates predate the current outcome; flat histories
    # generate missing state, never an artificial high-conviction reversal.
    daily_var = returns.rolling(window).var().shift(1).where(lambda x: x > 1e-12)
    block_var = returns.rolling(SHORT_DAYS).sum().rolling(window).var().shift(1)
    ratio = block_var / (SHORT_DAYS * daily_var)
    market = returns.where(liquid).mean(axis=1)
    relative_loss = -returns.sub(market, axis=0).rolling(SHORT_DAYS).sum()
    state = (1.0 - ratio).clip(lower=0)
    if mode == "ablation":
        state = pd.DataFrame(1.0, index=returns.index, columns=returns.columns).where(ratio.notna())
    elif mode == "falsifier":
        state = (ratio - 1.0).clip(lower=0)
    score = relative_loss.clip(lower=0) * state
    score = score.where(market.rolling(STATE_DAYS).sum() > 0, 0.0, axis=0)
    return score, liquid
