"""frontier_20260911e_range_acceptance_escape — PENDING RESEARCH STRATEGY.
Experiment: frontier_20260911e_range_acceptance_escape
Preregistration SHA256: ba60992f66b9bf9f063fe5c2172872acbe5cbe63180d5b4a612f13caeaffe1cd
Mechanism: An upside escape following sustained acceptance inside prior daily ranges has better continuation than a generic breakout.
Nearest incumbent: path-efficient trend / unimplemented Frontier-D range_acceptance
...
def signals(data, window, mode="base"):
    ...
    high, low = _field(data, "high"), _field(data, "low")
    valid_range = np.isfinite(high) & np.isfinite(low) & (high >= close) & (low <= close) & (high > low)
    high, low = high.where(valid_range), low.where(valid_range)
    observed = close.notna() & high.shift(1).notna() & low.shift(1).notna()
    accepted = ((close >= low.shift(1)) & (close <= high.shift(1))).astype(float).where(observed)
    acceptance = accepted.rolling(window).mean().shift(1)
    old = acceptance.shift(window)
    shared_valid = acceptance.notna() & old.notna()
    if mode == "ablation":
        acceptance = acceptance.where(~shared_valid, 1.0)
    elif mode == "falsifier":
        acceptance = old
    ...
    atr = tr.rolling(14).mean().where(lambda x: x > 1e-12)
    upper = high.rolling(STATE_DAYS).max().shift(1)
    escape = ((close - upper) / atr).clip(0, 1)
    score = (acceptance * escape).where(shared_valid).rolling(5).max()
