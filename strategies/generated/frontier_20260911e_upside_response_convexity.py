"""frontier_20260911e_upside_response_convexity — PENDING RESEARCH STRATEGY.
Experiment: frontier_20260911e_upside_response_convexity
Preregistration SHA256: aa671b257c19c4b8ba172564fbef05081f07350f85ec7aec4d0c0da98324e931
Mechanism: Assets whose response increases in large positive peer-market moves retain upside participation in established uptrends.
Nearest incumbent: C165 market timing / queued Frontier-C response_sign_asymmetry
...
def _peer_market(returns, liquid):
    observed = returns.where(liquid)
    total = observed.sort_index(axis=1).sum(axis=1, min_count=1)
    count = observed.notna().sum(axis=1)
    peers = pd.DataFrame(np.repeat(total.to_numpy()[:, None], len(returns.columns), axis=1),
                         index=returns.index, columns=returns.columns) - observed.fillna(0)
    n = pd.DataFrame(np.repeat(count.to_numpy()[:, None], len(returns.columns), axis=1),
                     index=returns.index, columns=returns.columns) - observed.notna().astype(int)
    return (peers / n.where(n >= 2)).where(np.isfinite(returns))


def _response(returns, peer, mask, window):
    valid = mask & np.isfinite(returns) & np.isfinite(peer)
    numerator = (returns * peer).where(valid, 0).rolling(window).sum()
    denominator = peer.pow(2).where(valid, 0).rolling(window).sum().where(lambda x: x > 1e-12)
    count = valid.astype(float).rolling(window).sum()
    return (numerator / denominator).where(count >= 5).shift(1)


def signals(data, window, mode="base"):
    _, liquid, returns = _context(data)
    peer = _peer_market(returns, liquid)
    threshold = peer.rolling(window).quantile(.75).shift(1).clip(lower=0)
    common = liquid & threshold.notna()
    tail = _response(returns, peer, common & (peer > threshold), window)
    middle = _response(returns, peer, common & (peer > 0) & (peer <= threshold), window)
    state = tail - middle
    if mode == "ablation":
        state = _response(returns, peer, common, window)
    elif mode == "falsifier":
        state = -state
    score = (state / 2).clip(0, 1).where(tail.notna() & middle.notna())
