"""PENDING RESEARCH STRATEGY — frontier_20260912i_conditional_decoupling.
Experiment: frontier_20260912i_conditional_decoupling
Preregistration SHA256: 60410c3e4dcd707ffe7196b67b3b45238ca80579ec50393a0ebb0f76d64a355b
Mechanism: marginal-vs-conditional residual dependence decoupling.
...
def _corr_partial(frame):
    ...
    if cov.ndim != 2 or not np.isfinite(cov).all():
        return None, None
    var = np.diag(cov)
    if np.any(var <= 1e-12):
        return None, None
...
def signals(data, window, mode="base"):
    ...
    for i, dt in enumerate(pd.DatetimeIndex(close.index)):
        ...
        marginal = pd.Series(np.abs(corr)[mask].reshape(n, n-1).mean(axis=1), index=active)
        conditional = pd.Series(np.abs(partial)[mask].reshape(n, n-1).mean(axis=1), index=active)
        spread = marginal - conditional
        if mode == "base":
            node = spread.rank(pct=True, method="average")
        elif mode == "ablation":
            node = (1.0 - marginal).rank(pct=True, method="average")
        elif mode == "falsifier":
            node = _rotate(spread.rank(pct=True, method="average"))
        else:
