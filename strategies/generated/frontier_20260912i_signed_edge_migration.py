"""frontier_20260912i_signed_edge_migration — PENDING RESEARCH STRATEGY.
Experiment: frontier_20260912i_signed_edge_migration
Preregistration SHA256: 15485ab84a1a660bbd165edc6b3ac0c1c9e97c0baad2355697db7be3e029b420
Mechanism: signed residual-edge strength migration during network fragmentation
Nearest incumbent: topology_migration / edge-sign-turnover reserve
...
def _signed_stats(frame, min_periods):
    corr = frame.corr(min_periods=min_periods).replace([np.inf, -np.inf], np.nan)
    arr = corr.to_numpy(float).copy()
    np.fill_diagonal(arr, np.nan)
    pos = np.nanmean(np.clip(arr, 0.0, None), axis=1)
    neg = np.nanmean(np.clip(-arr, 0.0, None), axis=1)
    abs_level = np.nanmean(np.abs(arr), axis=1)
    global_level = float(np.nanmean(np.abs(arr))) if np.isfinite(arr).any() else np.nan
    idx = corr.index
    return (pd.Series(pos, idx), pd.Series(neg, idx), pd.Series(abs_level, idx), global_level)
...
def signals(data, window, mode="base"):
    ...
    for i in range(first, len(times)):
        ...
        cpos, cneg, cabs, cg = _signed_stats(cur, minp)
        ppos, pneg, pabs, pg = _signed_stats(prev, minp)
