"""PENDING RESEARCH STRATEGY — frontier_20260912j_range_shock_decoupling.
Experiment: frontier_20260912j_range_shock_decoupling
Preregistration SHA256: bc25d0007b54903ab8fc5bda717b05e62aea892079149274990d52d702c6f8c6
Mechanism: intraday realized-range shock co-exceedance decoupling.
...
def _context(data):
    ...
    liquid = _field(data, "is_liquid").eq(1) & close.notna() & high.notna() & low.notna()
    r = np.log(close / close.shift(1))
    market = r.where(liquid).sort_index(axis=1).mean(axis=1)
    residual = r.sub(market, axis=0)
    realized_range = np.log(high / low).where(lambda x: np.isfinite(x) & (x >= 0))
    return close, liquid, residual, realized_range


def _shock_centrality(frame):
    x = frame.dropna(how="any")
    n = x.shape[1]
    if len(x) < 16 or n < 3:
        return None
    q = x.quantile(.75)
    a = x.ge(q, axis=1).to_numpy(dtype=float)
    p = a.mean(axis=0)
    joint = (a.T @ a) / len(a)
...
def signals(data, window, mode="base"):
    close, liquid, residual, rr = _context(data)
    ordered = sorted(close.columns)
    res = residual.reindex(columns=ordered)
    ranges = rr.reindex(columns=ordered)
    ...
    for i, dt in enumerate(pd.DatetimeIndex(close.index)):
        ...
        if len(active) < 3:
            continue
        frame = ranges.iloc[i-window+1:i+1][active].dropna(how="any")
        if len(frame) < minp:
            continue
        if mode == "base":
            central = _shock_centrality(frame)
            if central is None:
                continue
            ...
            if central is None:
                continue
            node = central.rank(pct=True, method="average")
        else:
