"""Frontier-M dispersion curvature — activated from preexisting unmeasured reserve."""
...
def signals(data, window, mode="base"):
    ...
    rel_path = residual.rolling(window, min_periods=max(10, window // 2)).sum()
    dispersion = rel_path.where(liquid).std(axis=1, ddof=0)
    convergence = dispersion.shift(STATE_LAG) - dispersion
    curvature = convergence - convergence.shift(STATE_LAG)
    if mode == "base":
    ...
    laggard = (-rel_path).clip(lower=0.0)
    score = laggard.mul(state, axis=0).where(liquid, 0.0)
