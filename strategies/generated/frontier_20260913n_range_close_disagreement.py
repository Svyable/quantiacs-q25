"""Frontier-N range/close estimator disagreement — IMPLEMENTED, UNMEASURED.

Experiment: frontier_20260913n_range_close_disagreement
Preregistration SHA256: e347f223530bc1d4a7408e75bc2fd253191447a2b6c6d50724ddb9777c317651
Mechanism: close-to-close residual volatility rank minus Parkinson high-low range-vol rank.
...
import pandas as pd
import xarray as xr
COMPETITION_TYPE="crypto_daily_long"; LOOKBACK_DAYS=365; NAME_CAP=0.25; TREND_DAYS=21; EPS=1e-12
...
def signals(data,window,mode="base"):
    _,h,l,_,_,liquid,residual=_context(data); log_range=np.log(h/l).where(liquid); minp=max(10,window//2); park=np.sqrt((log_range.pow(2)/(4*np.log(2))).rolling(window,min_periods=minp).mean()); close_vol=residual.rolling(window,min_periods=minp).std(); range_rank=_pct_rank(park,liquid); close_rank=_pct_rank(close_vol,liquid); disagreement=close_rank-range_rank
    if mode=="base": node=disagreement
    elif mode=="ablation": node=1.0-range_rank
    elif mode=="falsifier": node=_rotate_eligible(disagreement,liquid)
...
PARAMS={"window":63,"top_k":5}; FAMILY="range_close_disagreement"
def strategy(data,params=None,mode="base"):
    ...
    if p["window"] not in {42,63,84} or p["top_k"]!=5: raise ValueError("outside preregistered Frontier-N grid")
