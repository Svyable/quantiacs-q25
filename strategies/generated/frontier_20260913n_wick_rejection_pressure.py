"""Frontier-N wick rejection pressure — IMPLEMENTED, UNMEASURED.

Experiment: frontier_20260913n_wick_rejection_pressure
Preregistration SHA256: 24cb05a6b1df1ce4f68a267e75d9171cfcd67b7da16912670be5b087ea442521
Mechanism: rolling within-bar rejection / close-location pressure weighted by dollar-volume rank.
...
def _context(data):
    o = _field(data, "open").where(lambda x: np.isfinite(x) & (x > 0))
    h = _field(data, "high").where(lambda x: np.isfinite(x) & (x > 0))
    l = _field(data, "low").where(lambda x: np.isfinite(x) & (x > 0))
    c = _field(data, "close").where(lambda x: np.isfinite(x) & (x > 0))
    v = _field(data, "vol").where(lambda x: np.isfinite(x) & (x > 0))
    ...
    return o, h, l, c, v, liquid, residual

def _pct_rank(values, eligible):
    ordered = values.sort_index(axis=1)
    mask = eligible.reindex(columns=ordered.columns).fillna(False)
    return ordered.where(mask).rank(axis=1, pct=True, method="first").reindex(columns=values.columns)
...
def _allocate(score, liquid, times, top_k):
    ...
    return xr.DataArray(weights.to_numpy(dtype=float), dims=("time","asset"), coords={"time": times, "asset": score.columns}, name=COMPETITION_TYPE)
...
def signals(data, window, mode="base"):
    o,h,l,c,v,liquid,residual = _context(data)
    span=(h-l).where(lambda x: x > EPS)
    lower=(np.minimum(o,c)-l)/span
    upper=(h-np.maximum(o,c))/span
    clv=(2*c-h-l)/span
    dollar=(c*v).where(liquid)
    volume_rank=_pct_rank(dollar, liquid)
    daily=(0.5*(lower-upper)+0.5*clv)*volume_rank
    minp=max(7,window//2)
    pressure=daily.rolling(window,min_periods=minp).mean()
    close_pressure=clv.rolling(window,min_periods=minp).mean()
    if mode=="base": node=pressure
    elif mode=="ablation": node=close_pressure
    elif mode=="falsifier": node=_rotate_eligible(pressure, liquid)
...
FAMILY = "wick_rejection_pressure"
...
def strategy(data, params=None, mode="base"):
    ...
    if p["window"] not in {42, 21, 63} or p["top_k"] != 5: raise ValueError("outside preregistered Frontier-N grid")
