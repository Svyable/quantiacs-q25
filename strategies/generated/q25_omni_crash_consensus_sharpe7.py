"""Q25 OMNI Crash Consensus: sparse cross-asset risk overlay on frozen Sharpe7.

The frozen Sharpe7 carrier is unchanged. Four equity stress experts are the same
fixed SPX/NDX tail-dependence and 21/126 realized-volatility-term signals used in
the frozen RiskCast experiment. The new mechanism is sparse conjunction:

    cross_majority = (at least 3 of 4 stock experts above their causal median)
                     AND
                     (crypto 21d momentum < 0)
                     AND
                     (crypto vol21 > vol126)

When the conjunction is inactive the overlay targets full carrier exposure.
When active, gross is reduced continuously according to the median stock-risk
magnitude with the already-frozen 0.35 floor and 0.10 immediate-cut band.

The 0.5 stock threshold is not fit: robust-unit 0.5 is the trailing median.
The crypto conditions use zero momentum and a unit volatility ratio, also
structural rather than fitted thresholds.

Stocks never select crypto identities. Cross-dataset Q25 admissibility remains
a hosted/preclear item.
"""
...
STRATEGY_ID="q25_omni_crash_consensus_sharpe7_v1"
EXPERIMENT_ID="omni_crash_consensus_20260919"
...
CRYPTO_VOL_FAST=21
CRYPTO_VOL_SLOW=126
CRYPTO_VOL_FAST_MIN=14
CRYPTO_VOL_SLOW_MIN=63
...
# Frozen promoted Sharpe7 carrier.
...
# Fixed stock experts from RiskCast.
...
def _tail_stress(ctx:dict[str,object])->pd.Series:
    zret=ctx["zret"]; market=ctx["market"]
    ...
    return components.median(axis=1,skipna=True).where(
        components.notna().sum(axis=1)>=2
    ).clip(0.0,1.0)
...
def _stock_experts(
    ...
    # Completed stock state is lagged one crypto day to avoid same-date close
    # timing ambiguity and is forward-filled across weekends before the lag.
    return experts.reindex(crypto_index).ffill().shift(1).clip(0.0,1.0)


def _crypto_fragility(crypto:xr.DataArray)->pd.DataFrame:
    close=_field(crypto,"close")
    liquid=_field(crypto,"is_liquid").fillna(0.0)>0.0
    market=_safe_log(close).diff().where(liquid).mean(axis=1)
    mom=market.rolling(CRYPTO_MOM,min_periods=CRYPTO_MOM_MIN).sum()
    vf=market.rolling(CRYPTO_VOL_FAST,min_periods=CRYPTO_VOL_FAST_MIN).std(ddof=1)
    vs=market.rolling(CRYPTO_VOL_SLOW,min_periods=CRYPTO_VOL_SLOW_MIN).std(ddof=1)
    valid=mom.notna()&vf.notna()&vs.notna()
    out=pd.DataFrame(index=market.index)
    out["market_mom21"]=mom
    out["vol21"]=vf
    out["vol126"]=vs
    out["fragile"]=valid&(mom<0.0)&(vf>vs)
    return out
...
def _activation(
    ...
    idx=experts.index
    risk=experts.median(axis=1,skipna=True)
    available=experts.notna().sum(axis=1)
    votes=(experts>0.5).sum(axis=1)
    fragile=fragility["fragile"].reindex(idx).fillna(False).astype(bool)
    ...
    if mode=="always_static":
        active=available>=2
        magnitude=risk
    elif mode=="stock_majority":
        active=(available>=3)&(votes>=3)
        magnitude=risk
    elif mode=="stock_unanimous":
        active=(available>=4)&(votes==4)
        magnitude=risk
    elif mode=="crypto_only":
        active=fragile
        magnitude=pd.Series(1.0,index=idx,dtype=float)
    elif mode=="cross_unanimous":
        active=(available>=4)&(votes==4)&fragile
        magnitude=risk
    elif mode=="cross_majority":
        active=(available>=3)&(votes>=3)&fragile
        magnitude=risk
    elif mode=="inverted_cross":
        active=(available>=4)&(votes<=1)&fragile
        magnitude=1.0-risk
    else:
        raise ValueError(mode)

    return pd.to_numeric(magnitude.where(active,0.0),errors="coerce").fillna(0.0).clip(0.0,1.0)


def _banded_scale(activation:pd.Series)->pd.Series:
    desired=np.exp(math.log(RISK_FLOOR)*activation.astype(float).clip(0.0,1.0))
    desired=pd.Series(desired,index=activation.index,dtype=float).clip(RISK_FLOOR,1.0)
...
def calculate_weights(
    ...
    frame=base.to_pandas().astype(float)
    idx=pd.DatetimeIndex(frame.index)
    experts=_stock_experts(data["spx"],data["ndx"],idx)
    fragility=_crypto_fragility(crypto).reindex(idx)
    activation=_activation(experts,fragility,mode)
    scale=_banded_scale(activation).reindex(idx).fillna(1.0)
...
def strategy(data:dict[str,xr.DataArray])->xr.DataArray:
    return calculate_weights(data,mode="cross_majority").isel(time=-1,drop=True)
...
def run_research(output:str|None=None)->dict[str,object]:
    ...
    payload:dict[str,object]={
        ...
        "evidence_class":"NEW_PREREGISTERED_CRASH_CONSENSUS_MECHANISM",
        ...
        for fold_name,(start,end) in folds.items():
            ...
            for cost in (0.04,0.08,0.12):
                ...
                stat=qnstats.calc_stat(
                    d,w,slippage_factor=cost,points_per_year=365
