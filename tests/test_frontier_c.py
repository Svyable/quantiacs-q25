"""Regression tests for frontier_20260910c (synthetic mechanics only; no economic evidence)."""
...
CAMPAIGN = "frontier_20260910c"
FAMILIES = ("signed_risk_transition", "positive_tail_decay", "assimilation_acceleration")
...
def test_campaign_contract_and_preregistration_hashes():
    ...
    assert all("Frontier-A/B" in x["score_basis"] for x in slate)
    ...
    for path in preregs:
        ...
        if rows:
            assert all(x["preregistration_sha256"] == digest for x in rows)
        else:
    ...
    assert manifest["prior_development_evidence"] == ["frontier_20260910", "frontier_20260910b"]
...
def test_source_identity_and_declared_grid(family):
    ...
    assert sorted({x["params"]["window"] for x in rows if x["mode"] == "base"}) == [42, 63, 84]
    d = panel()
    m = module(family)
    for row in rows:
...
def test_frontier_c_does_not_reuse_dead_frontier_a_formulas():
    sources = "\n".join((ROOT / f"strategies/generated/{CAMPAIGN}_{f}.py").read_text() for f in FAMILIES)
    assert "flow_absorption" not in sources
    assert "forecast_surprise" not in sources
    assert 'FAMILY = "vol_curve"' not in sources
